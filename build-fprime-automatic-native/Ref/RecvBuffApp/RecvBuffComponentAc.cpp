...
namespace Ref {
  ...
  void RecvBuffComponentBase::loadParameters() {
  ...
  // ----------------------------------------------------------------------
    ...
    RecvBuffComponentBase :: RecvBuffComponentBase(const char* compName) :
  ...
  void RecvBuffComponentBase ::
    tlmWrite_PktState(const Ref::PacketStat& arg, Fw::Time _tlmTime)
  {
  ...
  void RecvBuffComponentBase ::
    tlmWrite_Sensor1(F32 arg, Fw::Time _tlmTime)
  {
    if (this->m_Tlm_OutputPort[0].isConnected()) {
      if (this->m_Time_OutputPort[0].isConnected() && _tlmTime ==  Fw::ZERO_TIME) {
         this->m_Time_OutputPort[0].invoke( _tlmTime);
  ...
  void RecvBuffComponentBase ::
    tlmWrite_Sensor2(F32 arg, Fw::Time _tlmTime)
  {
    if (this->m_Tlm_OutputPort[0].isConnected()) {
      if (this->m_Time_OutputPort[0].isConnected() && _tlmTime ==  Fw::ZERO_TIME) {
         this->m_Time_OutputPort[0].invoke( _tlmTime);
  ...
  void RecvBuffComponentBase ::
    tlmWrite_Parameter1(U32 arg, Fw::Time _tlmTime)
  {
  ...
  void RecvBuffComponentBase ::
    tlmWrite_Parameter2(I16 arg, Fw::Time _tlmTime)
  {
    // Check to see if it is the first time
...
#if FW_OBJECT_NAMES == 1
      const char* _formatString =
        "(%s) %s: Packet ID %u had checksum error";
#else
      const char* _formatString =
        "%s: Packet ID %u had checksum error";
#endif
...
#if FW_OBJECT_NAMES == 1
      const char* _formatString =
        "(%s) %s: BuffRecv Parameter %u was updated";
#else
      const char* _formatString =
        "%s: BuffRecv Parameter %u was updated";
#endif
